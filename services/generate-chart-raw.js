function generateChartRaw(data, options) {
  //   data = [
  //     {
  //       version: "1.0.1",
  //       pass: 1,
  ...
  let chart = {
    ...
    data: dataTemp,
    options: {
      legend: { display: data.versions ? true : false },
      title: {
  ...
  return `<div style="width:${options.width}px;height:${options.height}px;">
      <canvas id="chart-rtrs" width=${options.width} height=${
    options.height
  }></canvas>
      </div>
      <script src="https://cdn.jsdelivr.net/npm/chart.js@2.8.0"></script>
      <script>new Chart(document.getElementById("chart-rtrs"), ${JSON.stringify(
        chart
      )});</script>`;
}

module.exports = generateChartRaw;
